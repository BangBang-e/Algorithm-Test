...
let inputString: string = ""; //사용자로부터 받은 모든 입력을 저장하는 변수 선언
let inputLines: string[] = []; //입력을 개행 문자(\n) 기준으로 분리한 후 저장하는 배열 선언
let currentLine: number = 0; //현재 처리하고 있는 라인의 인덱스를 저장하는 변수 선언
...
function main() {
  let a: number = parseInt(readLine().trim(), 10);
  let b: number = parseInt(readLine().trim(), 10);
  let sum: number = a + b;
